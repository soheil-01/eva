...
pub const Tokenizer = struct {
    ...
    pub const TokenType = enum { Number, String, SemiColon, OpenBrace, CloseBrace, OpenPran, ClosePran, AdditiveOperator, MultiplicativeOperator, RelationalOperator, EqualityOperator, Identifier, SimpleAssign, ComplexAssign, Let, Comma, If, Else, True, False, Null, LogicalAnd, LogicalOr };
    pub const Token = struct { type: TokenType, value: []const u8 };

    const Spec = struct { re: []const u8, tokenType: ?TokenType };
    const spec = [_]Spec{ .{ .re = "^\\s", .tokenType = null }, .{ .re = "^//.*", .tokenType = null }, .{ .re = "^/\\*[\\s\\S]*?\\*/", .tokenType = null }, .{ .re = "^,", .tokenType = .Comma }, .{ .re = "^\\blet\\b", .tokenType = .Let }, .{ .re = "^\\bif\\b", .tokenType = .If }, .{ .re = "^\\belse\\b", .tokenType = .Else }, .{ .re = "^\\btrue\\b", .tokenType = .True }, .{ .re = "^\\bfalse\\b", .tokenType = .False }, .{ .re = "^\\bnull\\b", .tokenType = .Null }, .{ .re = "^\\d+", .tokenType = .Number }, .{ .re = "^\\w+", .tokenType = .Identifier }, .{ .re = "^[=!]=", .tokenType = .EqualityOperator }, .{ .re = "^=", .tokenType = .SimpleAssign }, .{ .re = "^[+\\-\\*/]=", .tokenType = .ComplexAssign }, .{ .re = "^\"[^\"]*\"", .tokenType = .String }, .{ .re = "^'[^']*'", .tokenType = .String }, .{ .re = "^;", .tokenType = .SemiColon }, .{ .re = "^\\{", .tokenType = .OpenBrace }, .{ .re = "^\\}", .tokenType = .CloseBrace }, .{ .re = "^\\(", .tokenType = .OpenPran }, .{ .re = "^\\)", .tokenType = .ClosePran }, .{ .re = "^[+\\-]", .tokenType = .AdditiveOperator }, .{ .re = "^[\\*/]", .tokenType = .MultiplicativeOperator }, .{ .re = "^[<>]=?", .tokenType = .RelationalOperator }, .{ .re = "^&&", .tokenType = .LogicalAnd }, .{ .re = "^\\|\\|", .tokenType = .LogicalOr } };
